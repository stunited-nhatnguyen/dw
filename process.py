...
def from_hdfs(time):
    try:
        ...
        for table in get_tables():
            df = spark.read.format('csv').options(header='true', inferSchema='true').load(f"data/{table}/{year}/{month}/{day}/")
            outputDf = df.withColumn("year", lit(year)).withColumn("month", lit(month)).withColumn("day", lit(day))
            outputDf.write.mode("append").partitionBy("year", "month", "day").parquet(f"hdfs://localhost:9000/datalake1/{table}")
        print('Process success!')
    except Exception as e:
        print(e)
