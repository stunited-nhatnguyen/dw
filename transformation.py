...
spark = SparkSession.builder.appName('Tranformation').getOrCreate()
...
def get_path_of_datalake(type):
    match type:
        case "lc":
            return f"hdfs://localhost:9000/datalake1"
        case _:
...
def get_path_of_datawarehouse(type):
    match type:
        case "lc":
            return f"hdfs://localhost:9000/dw"
        case _:
...
def execute(type, time):
    try:
        ...
        query.write.mode("overwrite").format("jdbc").option("url", "jdbc:postgresql://localhost:5432/metastore")\
