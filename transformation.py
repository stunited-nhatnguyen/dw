...
def execute(type, time):
    try:
        ...
        df = spark.read.format('parquet').options(header='true', inferSchema='true').load(f"s3a://{BUCKET}/{TRUST_PATH}/{year}/{month}/{day}/").createOrReplaceTempView("test")
        query = spark.sql("select *, 1 as Test from test")
        query.show(5)
